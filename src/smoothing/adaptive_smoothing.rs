...
pub fn smooth(data: &[f64], resolution: usize) -> Vec<f64> {
    ...
    let max_lag = (data.len() / 10).max(1);
    let mut acf = ACF::new(data.clone(), max_lag);
    let peaks = acf.find_peaks();
    
    // If no peaks found, apply simple smoothing
    if peaks.is_empty() {
        let window_size = (data.len() / 5).max(2).min(data.len());
    ...
    // Ensure window_size is at least 1 and not larger than data length
    window_size = window_size.max(1).min(data.len());
...
fn binary_search(mut head: usize, mut tail: usize, data: &[f64], mut min_obj: f64, original_kurt: f64, mut window_size: usize) -> usize {
    ...
    head = head.min(data.len() - 1);
    tail = tail.min(data.len() - 1);
